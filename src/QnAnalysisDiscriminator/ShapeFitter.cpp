...
void ShapeFitter::Fit()
{
  ...
  const float left_external = mu - 15*sigma;
  const float left_internal = mu - 8*sigma;
  const float right_internal = mu + 8*sigma;
  const float right_external = mu + 15*sigma;
  ...
  histo_sgnl_ = SubtractBckgr(histo_all_, func_bckgr, left_external, right_external);
  chi2_bckgr_fit_ = func_bckgr->GetChisquare() / func_bckgr->GetNDF();
}
...
TH1F* ShapeFitter::SubtractBckgr(TH1F* histo, TF1* func, float left, float right)
{
  TH1F* histo_out = (TH1F*)histo->Clone();
  histo_out -> Sumw2();
