...
{
  const float left_external = ShapeFitter::mu - 15 * ShapeFitter::sigma;
...
//   sgnl_fit -> SetParameter(0, histo->Interpolate(ShapeFitter::mu));
...
//   sgnl_fit -> SetParameter(3, ShapeFitter::sigma);  
  ...
  sgnl_fit -> SetParameter(0, histo->Interpolate(ShapeFitter::mu)*ShapeFitter::sigma);
  sgnl_fit -> FixParameter(1, ShapeFitter::mu);
  sgnl_fit -> SetParameter(2, 0.);
  sgnl_fit -> SetParameter(3, ShapeFitter::sigma);  
...
//   TF1* sgnl_fit = new TF1("sgnl_fit", "[0]*TMath::Voigt(x, [1], [2])",      left, right);  
